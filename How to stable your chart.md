...
- **仅使用Vocaloid音源创作的曲目，且艺术家原文和标题原文均不包含所使用的Vocaloid音源名称时，应在艺术家原文后添加使用的音源名称**，格式为`Artist feat.Vocaloid - Title`。**使用了多个Vocaloid的情况下，以`&`符号作为分隔，且不用在其前后添加空格。**
	- `feat.`字段前面要用空格，后面不用空格

		> 举一个例子：`hinayukki@仕事してP feat.KAITO&MEIKO - 番凩-2019-`
- **若角色歌的官方元数据源没有给出能够追溯到真实存在的人的艺术家原文，则使用`%角色名% (CV: %角色配音演员姓名%)`格式表记。**
- **若歌曲为多人合作，且官方元数据源没有给出固定的艺术家原文表记形式，则使用`&`作为分隔，且需要在其前后添加空格**
...
### Creator
#### 规定
- **不允许出现超过字段最大长度限制`（255 char）`的Creator字段。**
...
- **对于有文件格式要求的内容，不允许单纯以修改后缀的形式上传至服务器。**
